...
subsets = int(sys.argv[5])
np.random.seed(subset_size)

unrelated_genes = gene_expression_data.columns.difference(epilepsy_genes)
sources_subsets = []
for i in range(subsets):
    random_genes = np.random.choice(unrelated_genes, subset_size, replace=False)
    sources_subsets.append(random_genes)

# Concatenate all the subsets in a (subsets * subset_size, resolution) array
data = np.array([])
for (i, subset) in enumerate(sources_subsets):
    sources = load_gene_expression_data(gene_expression_data, resolution, subset)
    data = np.concatenate((data, sources), axis=0) if data.size else sources

# Permute the data randomly across all axis
data_flattened = data.flatten()  # Flatten the array to 1D
permuted_data_flattened = np.random.permutation(data_flattened)  # Permute the flattened array
permuted_data = permuted_data_flattened.reshape(data.shape)  # Reshape it back to the original shape
...
for (i, subset) in enumerate(sources_subsets): 
    sources = permuted_data[i * subset_size: (i + 1) * subset_size]
    synergy = singular_spin_test(sources, target, target_perms, perm_index=int(sys.argv[1]) - 1, n_bins=n_bins, len_py=len_py)
    print(synergy)
    with open(f"shuffled_random_subsets_count={subsets}_size={subset_size}.csv", "a") as f:
